...
public class FeedbackDAO {
    ...
    try {
        ...
        if (searchParam != null && !searchParam.trim().isEmpty()) {
            query.append(" AND (f.comment LIKE ? OR c.fullName like ? OR p.name LIKE ?)");
        ...
        try ( ResultSet resultSet = preparedStatement.executeQuery()) {
            while (resultSet.next()) {
                ...
                feedback.setContent(resultSet.getString("comment"));
...
// Method to add a new feedback
    ...
    public void addFeedback(Feedback feedback) {
        String sql = "INSERT INTO Feedbacks (comment, customerID, productID) VALUES (?, ?, ?)";
    ...
    public void updateFeedback(Feedback feedback) {
        String sql = "UPDATE Feedbacks SET comment = ?, customerID = ?, productID = ? WHERE feedbackID = ?";
        try ( PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setString(1, feedback.getContent());
            ps.setInt(2, feedback.getCustomer().getCustomerID());
            ps.setInt(3, feedback.getProductID());
